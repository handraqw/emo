...
При этом `main()` допускает запуск только на Windows и завершает работу с ошибкой на других платформах.
...
На Windows система автоматически ищет `ffmpeg` в `PATH` или использует `imageio-ffmpeg`.
